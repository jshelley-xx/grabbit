...
import com.twcable.grabbit.jcr.AuthorizableProtoNodeDecorator
...
class AuthorizableProtoNodeDecoratorSpec extends Specification {
    ...
    AuthorizableProtoNodeDecorator theProtoNodeDecorator(boolean forUser, Closure configuration = null){
        ProtoNodeBuilder nodeBuilder = ProtoNode.newBuilder()
        nodeBuilder.setName("user1")
    ...
    def "Throws an InsufficientGrabbitPrivilegeException if JVM permissions are not present"() {
        when:
        final protoNodeDecorator = theProtoNodeDecorator(false) {
    ...
    def "writeToJcr() will create a new underlying User, and return it's node within a JcrNodeDecorator"(){
        ...
        final protoNodeDecorator = theProtoNodeDecorator(true) {
            ...
            it.getUserManager(session) >> Mock(UserManager) {
                ...
                1 * it.createUser('authorizableID', 'temp', new AuthorizablePrincipal('authorizableID'), '/home/users/auth_folder') >> newUser
            }
        }
